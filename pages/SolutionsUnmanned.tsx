    ...
    <>
      ...
      <Section>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-16">
          {[
            { name: "NODE Series", desc: "Tactical Multi-rotor for urban ISR." },
            { name: "SCOUT", desc: "Long-range fixed-wing surveillance." },
            { name: "DENGGI Series", desc: "Specialized heavy-lift industrial vector." },
            { name: "HAN2", desc: "Next-gen hybrid propulsion platform." },
          ].map((platform, idx) => (
            <div key={idx} className="bg-slate-800 border border-slate-700 flex flex-col hover:border-sky-500 transition-colors">
              <div className="h-48 bg-slate-700 overflow-hidden relative">
                 <img src={`https://picsum.photos/400/300?random=${idx + 10}`} alt={platform.name} className="w-full h-full object-cover opacity-70" />
              </div>
